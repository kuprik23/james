...
const { iotManager } = require('./iot/iot-manager');

// Import security modules
const { securityCore } = require('./security/security-core');
const { antiMalware } = require('./security/anti-malware');
const { antiRansomware } = require('./security/anti-ransomware');
...
apiRouter.get('/llm/providers', (req, res) => {
  res.json(llmProvider.getProviders());
});

apiRouter.post('/llm/switch', (req, res) => {
  try {
...
apiRouter.post('/llm/key', (req, res) => {
  try {
...
apiRouter.get('/llm/ollama/models', async (req, res) => {
...
apiRouter.post('/agents/switch', (req, res) => {
  try {
...
apiRouter.post('/agents/create', (req, res) => {
  try {
...
apiRouter.post('/agents/customize', (req, res) => {
  try {
...
apiRouter.get('/tools/:category', (req, res) => {
...
apiRouter.post('/scan/ports', async (req, res) => {
  try {
...
apiRouter.get('/scan/system', async (req, res) => {
  try {
...
apiRouter.get('/scan/network', async (req, res) => {
  try {
...
// IoT endpoints
apiRouter.get('/iot/protocols', (req, res) => {
...
apiRouter.post('/iot/devices', (req, res) => {
  try {
    const { id, config } = req.body;
...
apiRouter.post('/iot/devices/:deviceId/connect', async (req, res) => {
  try {
...
apiRouter.post('/iot/devices/:deviceId/disconnect', async (req, res) => {
  try {
...
apiRouter.delete('/iot/devices/:deviceId', (req, res) => {
  try {
...
apiRouter.post('/iot/discover', async (req, res) => {
  try {
...
apiRouter.post('/iot/api', (req, res) => {
  try {
    const { id, config } = req.body;
...
apiRouter.post('/iot/api/:deviceId/call', async (req, res) => {
  try {
...
apiRouter.get('/security/status', (req, res) => {
  res.json({
...
apiRouter.post('/security/scan/malware', async (req, res) => {
  try {
    const { path } = req.body;
    const result = await antiMalware.performFullScan({ directories: [path || process.cwd()] });
    res.json(result);
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
...
apiRouter.get('/security/quarantine', (req, res) => {
  res.json({
...
apiRouter.post('/security/ransomware/monitor', (req, res) => {
  try {
...
apiRouter.get('/security/audit', (req, res) => {
  try {
...
apiRouter.post('/config/save', (req, res) => {
  try {
...
io.on('connection', (socket) => {
  ...
  socket.on('executeTool', async (data) => {
    try {
      ...
      socket.emit('toolError', { tool: data.toolId, error: error.message });
    }
  });
  
  // IoT events
  socket.on('iotConnect', async (data) => {
    try {
      const result = await iotManager.connectDevice(data.deviceId);
      socket.emit('iotConnected', result);
    } catch (error) {
      socket.emit('iotError', { error: error.message });
    }
  });
  
  socket.on('iotDisconnect', async (data) => {
    try {
...
llmProvider.on('providerChanged', (data) => io.emit('providerChanged', data));
llmProvider.on('chatStart', (data) => io.emit('chatStart', data));
llmProvider.on('chatComplete', (data) => io.emit('chatComplete', data));

agentManager.on('agentChanged', (data) => io.emit('agentChanged', data));
agentManager.on('historyCleared', () => io.emit('historyCleared'));

iotManager.on('deviceConnected', (data) => io.emit('iotDeviceConnected', data));
iotManager.on('deviceDisconnected', (data) => io.emit('iotDeviceDisconnected', data));
iotManager.on('deviceError', (data) => io.emit('iotDeviceError', data));
iotManager.on('discoveryProgress', (data) => io.emit('iotDiscoveryProgress', data));
...
antiMalware.on('malware_detected', (alert) => {
...
rateLimiter.on('ip_blacklisted', (data) => {
